...
def prompt_stats(path: str) -> dict:
    ...
    return {"path": str(prompt_path), "sha256": digest, "chars": len(text), "words": len(re.findall(r"\S+", text))}
...
def unique_llm_prompt_path(local_prompt: str) -> Path:
    ...
    candidate = local_path.parent / f"{local_path.stem}.llm.prompt"
    if not candidate.exists():
        return candidate
    for suffix in range(2, 100):
        candidate = local_path.parent / f"{local_path.stem}.llm-{suffix}.prompt"
        if not candidate.exists():
            return candidate
    return local_path.parent / f"{local_path.stem}.llm-{os.getpid()}.prompt"
...
def prompt_matches_registry(project_folder: str, entry_prompt: str, prompt: str) -> bool:
    ...
    if not entry_prompt:
        return False
    if entry_prompt in (prompt, registry_prompt_value(project_folder, prompt)):
        return True
    project_dir = get_project_dir(project_folder)
    entry_path, prompt_path = Path(entry_prompt), Path(prompt)
...
def promote_llm_winner(project_folder: str, local_prompt: str, llm_prompt: str) -> int:
    ...
    replacement, changed = registry_prompt_value(project_folder, llm_prompt), 0
    for entry in agents:
        if prompt_matches_registry(project_folder, entry.get("prompt_file", ""), local_prompt) and entry.get("prompt_file") != replacement:
...
def build_log_entry(*, working_dir: str, local_prompt: str, llm_prompt: str, winner: str, reason: str) -> dict:
    if winner not in WINNERS:
        raise ValueError(f"winner must be one of: {', '.join(sorted(WINNERS))}")
    return {
        "date": datetime.now(timezone.utc).isoformat(), "working_directory": os.path.abspath(working_dir),
        "judge": "llm", "winner": winner, "reason": reason,
        "local": prompt_stats(local_prompt), "llm": prompt_stats(llm_prompt),
    }
...
def cmd_log(args) -> int:
    ...
    try:
        entry = build_log_entry(working_dir=args.working_dir, local_prompt=args.local_prompt, llm_prompt=args.llm_prompt, winner=args.winner, reason=args.reason)
    except (OSError, ValueError) as exc:
        print(f"Error: {exc}", file=sys.stderr)
        return 1
    path = append_log(project_folder, entry)
    promoted = promote_llm_winner(project_folder, args.local_prompt, args.llm_prompt) if args.winner == "llm" else 0
